...
\begin{frame}
    ...
    \begin{block}{}
        ...
        \begin{enumerate}
            ...
            \item Several \alert{bugs} in the original Fortran code are \alert{patched}.
        \end{enumerate}
    \end{block}
...
\begin{frame}
    ...
    The~\alert{$\Lambda$-poisedness} of~$\xpt[k]$ plays a crucial role when bounding the model error.
...
\begin{frame}
            ...
            \begin{center}
                \begin{tikzpicture}[scale=0.85]
                    ...
                    \begin{scope}
                        ...
                        \uncover<10>{\fill[pattern=crosshatch,opacity=0.7] (0,0) circle (3);}
                    \end{scope}
    ...
    \begin{enumerate}
        \item The \alert<1-5>{standard} approach vs.\ our \alert<6->{new} one.
        \uncover<10>{\item The feasible region for~$\tstep[k]$ is \alert{wider} in the new approach.}
    \end{enumerate}
\end{frame}
...
\begin{frame}
    ...
    % TODO: \footnote{See \textcite{Conn_Gould_Toint_2000}.}
...
\begin{frame}
    ...
    \begin{block}{Derivative-free symmetric Broyden update \parencite{Powell_2004b}}
        ...
        \begin{align*}
            \min_{Q}    & \quad \textcolor{RedViolet}{\norm[\big]{\nabla^2 \objm[k - 1] - \nabla^2 Q}_{\mathsf{F}}}\\
...
\begin{frame}
    ...
    \begin{enumerate}
        \item What is~$\lm[k]$? 
        We choose the least-squares Lagrange multiplier.
        \item Which merit function should we use? 
        It uses the~$\ell_2$-merit function.
        \item How to update the penalty parameter? 
        It incorporates
        \begin{enumerate}
            \item a theoretical value that ensures the exactness of the merit function, and
            \item a strategy used by Powell in COBYLA.
        \end{enumerate}
        \item What if the set~$\xpt[k]$ is almost nonpoised?
        Central difficulty in the implementation of DFO methods.
    \end{enumerate}
    ...
    Chapter~5 and~6 of the thesis address these questions in detail.
\end{frame}
...
\begin{frame}
    ...
    \begin{enumerate}[<+->]
        ...
        \item adding noise to~$\obj$, with~$\sigma = 10^{-6}$.
    \end{enumerate}
    ...
    \begin{center}
        ...
        \only<2>{\drawperformanceprofiles{{"COBYQA","BOBYQA","COBYLA","PY-BOBYQA"}}{noisy-1-50-6-perf-bobyqa-cobyla-cobyqa-py-bobyqa-b.csv}{4}}
    \end{center}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        ...
        \only<2>{\drawperformanceprofiles{{"COBYQA","COBYLA"}}{noisy-1-50-6-perf-cobyla-cobyqa-qo.csv}{4}}
    \end{center}
\end{frame}
